if not LoadResourceFile(lib.name, 'web/build/index.html') then
	error('Unable to load UI. Build ox_doorlock or download the latest release.\n	^3github.com/overextended/ox_doorlock/releases/latest/download/ox_doorlock.zip^0')
end
...
RegisterNetEvent('ox_doorlock:setState', function(id, state, source, data)
	...
	if Config.Notify and source == cache.serverId then
		if state == 0 then
			ESX.ShowNotification("Porte déverrouillée")
			--lib.notify({
			--	type = 'success',
			--	icon = 'unlock',
			--	description = locale('unlocked_door'), position = 'bottom'
			--})
		else
			ESX.ShowNotification("Porte verrouillée")
			--lib.notify({
			--	type = 'success',
			--	icon = 'lock',
			--	description = locale('locked_door'), position = 'bottom'
			--})
		end
	end
