...
RegisterNUICallback("shop:Close", function()
    ...
    -- Libérer le joueur (fix bug bloqué après sortie)
    local playerPed = PlayerPedId()
...
RegisterNUICallback("shop:SaveOutfit", function()
    ...
    ESX.TriggerServerCallback("MakeUP:removeMoney", function(isAllow)
        if isAllow then
            ...
            ESX.ShowNotification('Tu as payé ~g~50$~w~ en liquide')
            clearInterface()
            ...
            ESX.ShowNotification("Vous n'avez pas ~r~50$~w~ en liquide")
            
            -- Fermer le menu et annuler les changements
            isInHairshop = false
            DisplayRadar(true)
            ...
            -- Recharger la skin précédente depuis la DB
            ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
                TriggerEvent('skinchanger:loadSkin', skin)
                TriggerEvent("esx:refreshSkin")
            end)
