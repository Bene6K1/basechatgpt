local TextPanels = {
    ...
    Text = {
        Title = { X = 8, Y = 10, Scale = 0.15 },
        Left = { X = 8, Y = 40, Scale = 0.13 },
        Right = { X = 8, Y = 40, Scale = 0.13 },
    },
}
...
function RageUI.Info(Title, RightText, LeftText)
    ...
    if Title ~= nil then
        RenderText("~h~" .. Title .. "~h~", 400 + 82, 75, 0, 0.30, 255, 255, 255, 255, 0)
    end
    if RightText ~= nil then
        RenderText(table.concat(RightText, "\n"), 400 + 80, Title ~= nil and 105 or 7, 0, 0.25, 255, 255, 255, 255, 0)
    end
    if LeftText ~= nil then
        RenderText(table.concat(LeftText, "\n"), 400 + 400 + 70, Title ~= nil and 105 or 7, 0, 0.22, 255, 255, 255, 255, 2)
    end
    RenderRectangle(400 + 75, 70, 400, Title ~= nil and 50 + (LineCount * 20) or ((LineCount + 1) * 20), 0, 0, 0, 160)
end

---
--- Generated by EmmyLua(github.com/EmmyLua)
--- Created by iTexZ.
...
function RageUI.RenderCaissePreview(Dictionary, Texture)
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() then
            local spriteWidth = (TextPanels.Background.Width + CurrentMenu.WidthOffset) * 0.7  -- Ajustez le facteur de réduction selon vos besoins
            local spriteHeight = (TextPanels.Background.Height + 200) * 0.7  -- Ajustez le facteur de réduction selon vos besoins
            RenderSprite(Dictionary, Texture, 476, 200, spriteWidth, spriteHeight, 0, 255, 255, 255, 255)
            RageUI.StatisticPanelCount = RageUI.StatisticPanelCount + 1
        end
    end
end
