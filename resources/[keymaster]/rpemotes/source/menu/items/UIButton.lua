...
function RageUI.Button(Label, Description, Style, Enabled, Action, Submenu)
    ...
    if CurrentMenu ~= nil and CurrentMenu() then
        ...
        if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
            ...
            local ibg = CurrentMenu.ItemBackgroundColor or { R = 21, G = 25, B = 31, A = 255 }
            RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 30, SettingsButton.SelectedSprite.Height - 3, ibg.R, ibg.G, ibg.B, ibg.A)
            ...
            if Active then 
                RenderRectangle(CurrentMenu.X + 15, CurrentMenu.Y + SettingsButton.SelectedSprite.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset, SettingsButton.SelectedSprite.Width + CurrentMenu.WidthOffset - 446, SettingsButton.SelectedSprite.Height - 3, CurrentMenu.ButtonColor.R, CurrentMenu.ButtonColor.G, CurrentMenu.ButtonColor.B, CurrentMenu.ButtonColor.A)
            end
