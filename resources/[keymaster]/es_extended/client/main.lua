...
local function shouldESXCloseLoadingScreen()
	local waitForReady = GetConvarInt and GetConvarInt('loadscreen:waitForReady', 1) == 1
	local loadingResourceStarted = false

	if GetResourceState then
		local ok, state = pcall(GetResourceState, 'loading')
		if ok and state == 'started' then
			loadingResourceStarted = true
		end
	end

	-- On ne laisse ESX fermer le loading que si aucun custom loader n'est en train
	-- de gérer la transition, ou si l'admin a désactivé le mode "waitForReady".
	return not (waitForReady and loadingResourceStarted)
end

local esxShouldShutdownLoading = shouldESXCloseLoadingScreen()

AddEventHandler("skinchanger:modelLoaded", function()
    if firstCall == false then
		if esxShouldShutdownLoading then
...
AddEventHandler('esx:removeInventoryItem', function(item, identifier)
	...
	for i = 1, #ESX.PlayerData.inventory, 1 do
		local invItem = ESX.PlayerData.inventory[i]

		if invItem and invItem.name == item.name and (not identifier or (item.unique and invItem.extra and invItem.extra.identifier == identifier)) then
			if item.label and item.count then
