...
function ESX.SavePlayer(xPlayer, cb)
	...
	if xPlayer then
		...
		table.insert(asyncTasks, function(cb)
			...
			-- IMPORTANT : Ne PAS sauvegarder inventory quand ox_inventory est activé
			-- ox_inventory gère lui-même la colonne inventory dans la BDD
			local query, params
			if Config.CustomInventory == "ox" then
				-- Sans inventory : ox_inventory gère cette colonne
				query = 'UPDATE users SET accounts = @accounts, playerName = @playerName, permission_group = @permission_group, permission_level = @permission_level, job = @job, job2 = @job2, job_grade = @job_grade, job2_grade = @job2_grade, loadout = @loadout, position = @position, ip = @ip WHERE identifier = @identifier'
				params = {
				...
				-- Avec inventory : système ESX classique
				query = 'UPDATE users SET accounts = @accounts, playerName = @playerName, permission_group = @permission_group, permission_level = @permission_level, job = @job, job2 = @job2, job_grade = @job_grade, job2_grade = @job2_grade, inventory = @inventory, loadout = @loadout, position = @position, ip = @ip WHERE identifier = @identifier'
				params = {
					['@identifier'] = xPlayer.identifier,
					['@playerName'] = xPlayer.getName(),
					['@permission_group'] = xPlayer.group,
...
function ESX.toConsole(data)
	...
	print(('[^5%s^0] => ' .. data):format("N E V A"))
end
...
function ESX.SetDensity(settings)
    ...
    TriggerClientEvent('es_extended:updateDensity', -1, settings)
end
