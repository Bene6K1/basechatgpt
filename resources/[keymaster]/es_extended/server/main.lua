...
function LoadUser(source, identifier)
	...
	table.insert(tasks, function(cb)
		...
		-- IMPORTANT : Ne PAS charger l'inventaire si ox_inventory est activé
		-- ox_inventory gère lui-même le chargement de l'inventaire
		if Config.CustomInventory == "ox" then
			-- ox_inventory gère l'inventaire, on laisse vide pour ESX
			userData.inventory = {}
		else
			-- Système ESX classique : charger l'inventaire depuis la BDD
	...
	Async.parallel(tasks, function(results)
		...
		local playerFirstname = xPlayer.firstname
		local playerLastname = xPlayer.lastname
		CreateThread(function()
			Wait(3000)
			TriggerClientEvent('chat:addMessage', playerId, {
				color = {11, 118, 254},
				multiline = true,
				args = {'NEVA', 'Bienvenue ' .. playerFirstname .. ' ' .. playerLastname .. ' sur NEVA RP !'}
			})
		end)
	end)
end
