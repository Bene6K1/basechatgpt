function CreatePlayer(source, identifier, userData)
    ...
    function self.addMoney(money)
        -- IMPORTANT : Si ox_inventory est activé, utiliser ox_inventory
        if Config.CustomInventory == "ox" then
            local success, response = exports.ox_inventory:AddItem(self.source, 'money', money)
            return success
        end
        
        -- Système ESX classique
        return self.addAccountMoney('cash', money)
    end
    
    function self.removeMoney(money)
        -- IMPORTANT : Si ox_inventory est activé, utiliser ox_inventory
        if Config.CustomInventory == "ox" then
            local success, response = exports.ox_inventory:RemoveItem(self.source, 'money', money)
            return success
        end
        
        -- Système ESX classique
        return self.removeAccountMoney('cash', money)
    end
    ...
    function self.addAccountMoney(accountName, money)
        ...
        if money > 0 then
            ...
            if account then
                ...
                account.money = newMoney
                self.triggerEvent('esx:setAccountMoney', account)
                -- Hook pour ox_inventory
                TriggerEvent('esx:onAddAccountMoney', self.source, account, money)
            end
        end
    end
    ...
    function self.removeAccountMoney(accountName, money)
        ...
        if money > 0 then
            ...
            if account then
                -- Hook pour ox_inventory (AVANT la modification)
                TriggerEvent('esx:onRemoveAccountMoney', self.source, account, money)
    ...
    function self.addInventoryItem(name, count, extra)
        -- IMPORTANT : Si ox_inventory est activé, utiliser ox_inventory au lieu d'ESX
        if Config.CustomInventory == "ox" then
            -- ox_inventory AddItem attend : (source, item, count, metadata, slot)
            local success, response = exports.ox_inventory:AddItem(self.source, name, count, extra)
            return success
        end
    ...
    function self.removeInventoryItem(name, count, identifier)
        -- IMPORTANT : Si ox_inventory est activé, utiliser ox_inventory au lieu d'ESX
        if Config.CustomInventory == "ox" then
            -- ox_inventory RemoveItem attend : (source, item, count, metadata, slot)
            local success, response = exports.ox_inventory:RemoveItem(self.source, name, count)
            return success
        end
