    ...
    window.addEventListener('message', function(event) {
        ...
        switch (data.action) {
            ...
            case 'togglePhoneNUI':
                ...
                togglePhoneUI(data.showNUI, true);
                break;
                ...
                var confirmIcon = $("<img>", {
                    ...
                    click: function() {
                        ...
                        if (!isNaN(numericValue) && numericValue.toString() === inputValue && numericValue >= 1) {
                            $.post('https://nv_drugs/dialogBack', JSON.stringify({ action: "sell", ped: ped, itemName: selectedItemName, inputValue: numericValue, count: npcQuantity[itemToSellName], isWholesale: false }))
        ...
        if (!isTyping) {
            ...
            if (response.action !== null) {
                ...
                if (response.endInteraction !== null && response.endInteraction) {
                    $.post('https://nv_drugs/successInteraction', JSON.stringify(itemSellData))
                } else {
                    $.post('https://nv_drugs/dialogBack', JSON.stringify({ action: response.action }))
                }
