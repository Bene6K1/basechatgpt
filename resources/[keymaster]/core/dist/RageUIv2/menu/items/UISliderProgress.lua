  ...
  if CurrentMenu ~= nil then
    if CurrentMenu then
      ...
      if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
        ...
        if Selected then
          -- Background item actif (Tactical Dark theme)
          RenderRectangle(
            CurrentMenu.X + 10,
            CurrentMenu.Y
        ...
        if Enabled == true or Enabled == nil then
          if Selected then
            ...
            if Style.RightLabel ~= nil and Style.RightLabel ~= "" then
              RightOffset = MeasureStringWidth(Style.RightLabel, 0, 0.35)
              -- RightLabel inactif gris clair (Tactical Dark theme)
              RenderText(
                Style.RightLabel,
                CurrentMenu.X
                  + SettingsButton.RightText.X
        ...
        if Enabled == true or Enabled == nil then
          if Selected then
            RenderText(
              Label,
              CurrentMenu.X + 20 + SettingsButton.Text.X + LeftBadgeOffset,
        ...
        if type(Style.ProgressBackgroundColor) == "table" then
          -- Progress background gris foncé (Tactical Dark theme)
          RenderRectangle(
        ...
        if type(Style.ProgressColor) == "table" then
          -- Progress bar gris clair (Tactical Dark monochrome)
          RenderRectangle(
...
function RageUI.SliderProgress(Label, Progress, Description, Style, Enabled, Actions)
  return Items:SliderProgress(Label, Progress, Description, Style, Enabled, Actions)
end
