...
function RageUI.IsMouseInBounds(X, Y, Width, Height)
  local MX, MY =
    math.round(GetControlNormal(2, 239) * 1920) / 1920,
  ...
  if CurrentMenu ~= nil then
    if CurrentMenu and CurrentMenu.Display.Header then
      ...
      RenderText(
        ...
        CurrentMenu.Y + RageUI.Settings.Items.Title.Text.Y,
    ...
    if CurrentMenu and CurrentMenu.Display.Subtitle then
      ...
      if CurrentMenu.Subtitle ~= "" then
        ...
        RenderText(
          ...
          CurrentMenu.Y + RageUI.Settings.Items.Subtitle.Text.Y + RageUI.ItemOffset,
          0,
        ...
        if CurrentMenu.Display.PageCounter then
          if CurrentMenu.PageCounter == nil then
            RenderText(
              ...
              CurrentMenu.Y + RageUI.Settings.Items.Subtitle.PreText.Y + RageUI.ItemOffset,
              0,
        ...
        RageUI.ItemOffset = RageUI.ItemOffset
          + RageUI.Settings.Items.Subtitle.Background.Height
          + 10
      end
    end
  end
...
function RageUI.Background()
  ...
  if CurrentMenu ~= nil then
    if CurrentMenu and CurrentMenu.Display.Background then
      ...
      RenderRectangle(
        CurrentMenu.X,
        CurrentMenu.Y + RageUI.Settings.Items.Background.Y + 10 + CurrentMenu.SubtitleHeight,
    ...
    if CurrentMenu then
      ...
      RenderRectangle(
        ...
        CurrentMenu.Y
          ...
          + RageUI.ItemOffset,
        Description.Background.Width + CurrentMenu.WidthOffset,
      ...
      RenderText(
        ...
        CurrentMenu.X + Description.Text.X,
        CurrentMenu.Y + Description.Text.Y + 13 + CurrentMenu.SubtitleHeight + RageUI.ItemOffset,
        0,
  ...
  if CurrentMenu ~= nil then
    if CurrentMenu then
      ...
      if CurrentMenu.Display.InstructionalButton then
        ...
        DrawScaleformMovieFullscreen(CurrentMenu.InstructionalScaleform, 255, 255, 255, 80, 0)
      end
...
function RageUI.ItemsMouseBounds(CurrentMenu, Selected, Option, SettingsButton)
  ...
  if Hovered and not Selected then
    -- Hover effect gris subtil (Tactical Dark theme) - fix white hover bug
    RenderRectangle(
      CurrentMenu.X,
...
function RageUI.IsVisible(Menu, Items, Panels)
  ...
  if
    ...
    and (UpdateOnscreenKeyboard() ~= 3)
  then
    RageUI.Banner()
    RageUI.Subtitle()
    if Items ~= nil then
      Items()
    elseif Menu.func ~= nil then
      Menu.func(Items)
    end
    RageUI.Background()
    RageUI.Navigation()
...
function RageUI.PercentagePanel(Percent, HeaderText, MinText, MaxText, Action, Index)
  return Panels:PercentagePanel(Percent, HeaderText, MinText, MaxText, Action, Index)
end