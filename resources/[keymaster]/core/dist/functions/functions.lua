...
function CreateInteractionPoint(coords, text, onPress, options)
    ...
    local id = exports['nv_interact']:addInteractionPoint({
        coords = coords,
        dist = opts.dist,
        key = opts.key,
        message = text,
...
function ClearAllInteractionPoints()
    ...
    if activeInteractionPoints then
        for k, id in pairs(activeInteractionPoints) do
            exports['nv_interact']:removeInteractionPoint(id)
        end
...
function RespawnPed(ped, spawn)
	...
	TriggerServerEvent('playerSpawned')  -- Pour ox_inventory : notifier le serveur que le spawn est complet
    ...
    while true do
        ...
        for k,v in pairs(ZonesListe) do
            ...
            if distance > v.Dist then 
                if activeInteractionPoints[k] then
                    RemoveInteractionPoint(activeInteractionPoints[k])
                    activeInteractionPoints[k] = nil
                end
                goto continue 
            end
            ...
            if distance <= 1.5 then
                if not activeInteractionPoints[k] then
                    local message = v.ActionText or "INTERAGIR"
                    local icon = v.Icon or "fa-hand-pointer"
                    activeInteractionPoints[k] = CreateInteractionPoint(
                        position,
                        message,
                        function() v.Action() end,
                        { 
                            icon = icon, 
                            dist = 1.5,
                            ped = v.Ped,
...
exports('GetWeaponPerm', function (name)
    return SunnyConfigServ.PermanantWeapon[name]
end)