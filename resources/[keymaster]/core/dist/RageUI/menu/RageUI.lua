...
RageUI.Settings = {
    ...
    Items = {
        ...
        Subtitle = {
            ...
            Text = { X = 6, Y = 2, Scale = 0.25 },
            PreText = { X = 410, Y = 2, Scale = 0.27 },
        },
        Background = { Dictionary = "utils", Texture = "gradient_bgd", Y = 0, Width = 415 },
        Navigation = {
            Rectangle = { Width = 0, Height = 0 },
            Offset = 5,
            Arrows = { Dictionary = "utils", Texture = "shop_arrows_upanddown", X = 190, Y = -6, Width = 0, Height = 0 },
        },
        Description = {
            Bar = { Y = 4, Width = 415, Height = 4 },
            Background = { Dictionary = "utils", Texture = "gradient_bgd", Y = 4, Width = 415, Height = 30 },
            Text = { X = 8, Y = 5, Scale = 0.25 },
        },
    },
    Panels = {
        Grid = {
            Background = { Dictionary = "utils", Texture = "gradient_bgd", Y = 4, Width = 415, Height = 275 },
        ...
        Percentage = {
            Background = { Dictionary = "utils", Texture = "gradient_bgd", Y = 4, Width = 415, Height = 76 },
        ...
        if Value == true or Value == false then
            if Value then
                if RageUI.CurrentMenu ~= nil then
                    ...
                    Menu:UpdateInstructionalButtons(true)
                    Menu:UpdateCursorStyle();
                end
                RageUI.CurrentMenu = Menu
            else
                Menu:UpdateInstructionalButtons(false)
                RageUI.CurrentMenu = nil
            end
            Menu.Open = Value
            RageUI.Options = 0
...
function RageUI.CloseAll()
    if RageUI.CurrentMenu ~= nil then
        ...
        while parent ~= nil do
            ...
            parent:UpdateInstructionalButtons(false)
            parent = parent.Parent
        end
        RageUI.CurrentMenu:UpdateInstructionalButtons(false)
            ...
            if CurrentMenu.Sprite ~= nil then
                if CurrentMenu.Sprite.Dictionary ~= nil then
                    if CurrentMenu.Sprite.Dictionary == "color" then
                        local c = CurrentMenu.Sprite.Color or { R = 0, G = 0, B = 0, A = 216 }
                        RenderSprite(
                            "color",
                            "blank",
                            CurrentMenu.X,
                        ...
                        RenderSprite(
                            ...
                            CurrentMenu.Sprite.Texture or "interaction_bgd",
                            CurrentMenu.X,
                            CurrentMenu.Y,
                            RageUI.Settings.Items.Title.Background.Width + CurrentMenu.WidthOffset,
                            RageUI.Settings.Items.Title.Background.Height,
                    ...
                    local rect = CurrentMenu.Rectangle or { R = 0, G = 0, B = 0, A = 216 }
                    RenderRectangle(CurrentMenu.X, CurrentMenu.Y,
                        RageUI.Settings.Items.Title.Background.Width + CurrentMenu.WidthOffset,
                        RageUI.Settings.Items.Title.Background.Height, rect.R, rect.G, rect.B, rect.A)
                end
            else
                local rect = CurrentMenu.Rectangle or { R = 0, G = 0, B = 0, A = 216 }
                RenderRectangle(CurrentMenu.X, CurrentMenu.Y,
...
function RageUI.Subtitle()
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (CurrentMenu.Display.Subtitle) then
            RageUI.ItemsSafeZone(CurrentMenu)
            local config = (SunnyConfigServ and SunnyConfigServ.RageUI) or (Config and Config.RageUI) or {}
            if CurrentMenu.Subtitle ~= "" then
                local subtitleAlpha = config.menuBackgroundAlpha or 180
                if config.arrondi then
                    RenderSprite("utils", "backgroundsbanner", CurrentMenu.X, CurrentMenu.Y + RageUI.ItemOffset,
                        RageUI.Settings.Items.Subtitle.Background.Width + CurrentMenu.WidthOffset,
                        RageUI.Settings.Items.Subtitle.Background.Height + CurrentMenu.SubtitleHeight - 3, 0, 16, 16,
                        16, subtitleAlpha)
                else
                    RenderRectangle(CurrentMenu.X, CurrentMenu.Y + RageUI.ItemOffset,
                        RageUI.Settings.Items.Subtitle.Background.Width + CurrentMenu.WidthOffset,
                        RageUI.Settings.Items.Subtitle.Background.Height + CurrentMenu.SubtitleHeight - 3, 16, 16,
                        16, subtitleAlpha)
                end
                ...
                local subtitleItemGap = config.subtitleItemGap or 0
                RageUI.ItemOffset = RageUI.ItemOffset + RageUI.Settings.Items.Subtitle.Background.Height - subtitleItemGap
            end
        end
    end
end
...
function RageUI.Background()
    ...
    if CurrentMenu ~= nil then
        if CurrentMenu() and (CurrentMenu.Display.Background) then
            ...
            local config = (SunnyConfigServ and SunnyConfigServ.RageUI) or (Config and Config.RageUI) or {}
            local ItemsOffsetY = config.itemsOffsetY or 0
            local bgHeight = (RageUI.ItemOffset - 117) - ItemsOffsetY
            if bgHeight < 0 then bgHeight = 0 end
            local bgAlpha = config.menuBackgroundAlpha or 180
            if config.arrondi then
                -- Use the original rounded sprite to avoid corner artifacts.
                RenderSprite("utils", "background", CurrentMenu.X,
                    146 + CurrentMenu.Y + ItemsOffsetY + RageUI.Settings.Items.Background.Y + CurrentMenu.SubtitleHeight,
                    RageUI.Settings.Items.Background.Width + CurrentMenu.WidthOffset, bgHeight,
                    0, 16, 16, 16, bgAlpha)
            else
                RenderRectangle(CurrentMenu.X,
                    146 + CurrentMenu.Y + ItemsOffsetY + RageUI.Settings.Items.Background.Y + CurrentMenu.SubtitleHeight,
                    RageUI.Settings.Items.Background.Width + CurrentMenu.WidthOffset, bgHeight, 16, 16, 16,
                    bgAlpha)
                if config.simpleBar then
                    RenderRectangle(CurrentMenu.X,
                        CurrentMenu.Y + RageUI.Settings.Items.Background.Y + CurrentMenu.SubtitleHeight +
                        RageUI.ItemOffset +
                        27,
                        RageUI.Settings.Items.Background.Width + CurrentMenu.WidthOffset, 2, config.color.R,
...
function RageUI.Description()
    ...
    local config = (SunnyConfigServ and SunnyConfigServ.RageUI) or (Config and Config.RageUI) or {}
    if config.hideDescription then
        return
    end
    if CurrentMenu ~= nil and CurrentMenu.Description ~= nil then
        if CurrentMenu() then
            ...
            if config.arrondi then
                RenderSprite("utils", "backgroundsarrondi", CurrentMenu.X,
                    CurrentMenu.Y + Description.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 28,
                    Description.Background.Width + CurrentMenu.WidthOffset, CurrentMenu.DescriptionHeight, 0, 16, 16, 16,
                    125)
            else
                RenderRectangle(CurrentMenu.X,
                    CurrentMenu.Y + Description.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 28,
                ...
                if config.simpleBar then
                    RenderRectangle(CurrentMenu.X,
                        CurrentMenu.Y + Description.Background.Y + CurrentMenu.SubtitleHeight + RageUI.ItemOffset + 27,
                        Description.Background.Width + CurrentMenu.WidthOffset, 2, config.color.R,
                        config.color.G, config.color.B, config.color.A)
                end
            end
            ...
            RenderText(CurrentMenu.Description, CurrentMenu.X + Description.Text.X,
                ...
                Description.Background.Width + CurrentMenu.WidthOffset - 25.0)
        ...
        if CurrentMenu() then
            ...
            if RageUI.NextMenu ~= nil then
                if RageUI.NextMenu() then
                    ...
                    if CurrentMenu.InstructionalButtons then
                        for i = 1, #CurrentMenu.InstructionalButtons do
                            if CurrentMenu.InstructionalButtons[i] and #CurrentMenu.InstructionalButtons[i] == 2 then
                                local key = CurrentMenu.InstructionalButtons[i][1]:gsub("t_", "")
                                currentButtons[key] = CurrentMenu.InstructionalButtons[i][2]
                            end
                        end
                    end
                    ...
                    if next(currentButtons) then
                        for key, value in pairs(currentButtons) do
                            RageUI.NextMenu:AddInstructionButton({ key, value })
                        end
                    end
...
function RageUI.ItemsMouseBounds(CurrentMenu, Selected, Option, SettingsButton)
    ...
    local config = (SunnyConfigServ and SunnyConfigServ.RageUI) or (Config and Config.RageUI) or {}
    local ItemsOffsetY = config.itemsOffsetY or 0
    local ItemBaseY = config.itemBaseY or 20
    Hovered = RageUI.IsMouseInBounds(CurrentMenu.X + CurrentMenu.SafeZoneSize.X,
        ...
        RageUI.ItemOffset + ItemsOffsetY + ItemBaseY, SettingsButton.Rectangle.Width + CurrentMenu.WidthOffset, SettingsButton.Rectangle.Height)
