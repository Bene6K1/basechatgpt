...
local SettingsButton = {
  ...
  Text = { X = 8, Y = 25, Scale = 0.20 },
  LeftBadge = { Y = -2, Width = 40, Height = 40 },
  RightBadge = { X = 385, Y = -2, Width = 40, Height = 40 },
  RightText = { X = 420, Y = 1, Scale = 0.35 },
  SelectedSprite = {
    Dictionary = "utils",
      ...
      if CurrentMenu.Pagination.Minimum <= Option and CurrentMenu.Pagination.Maximum >= Option then
        ...
        if Enabled == true or Enabled == nil then
          if Selected then
            RenderText(
              Label,
              28 + CurrentMenu.X + SettingsButton.Text.X + LeftBadgeOffset,
        ...
        if Divider then
          RenderRectangle(
            28 + CurrentMenu.X + SettingsSlider.Divider.X + CurrentMenu.WidthOffset,
            CurrentMenu.Y
              + ItemsOffsetY
              + SettingsSlider.Divider.Y
              + CurrentMenu.SubtitleHeight
              + RageUI.ItemOffset,
        ...
        if Selected and (CurrentMenu.Controls.SliderLeft.Active or (CurrentMenu.Controls.Click.Active and LeftArrowHovered)) and not (CurrentMenu.Controls.SliderRight.Active or (CurrentMenu.Controls.Click.Active and RightArrowHovered)) then
          ...
          if (Actions.onSliderChange ~= nil) then
            Actions.onSliderChange(ProgressStart, Items[ProgressStart])
          end
        ...
        if Selected and (CurrentMenu.Controls.Select.Active or ((Hovered and CurrentMenu.Controls.Click.Active) and (not LeftArrowHovered and not RightArrowHovered))) then
          if (Actions.onSelected ~= nil) then
            Actions.onSelected(ProgressStart, Items[ProgressStart])
          end
