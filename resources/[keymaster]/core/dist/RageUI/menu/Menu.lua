function RageUI.CreateMenu(Title, Subtitle, X, Y, TextureDictionary, TextureName, R, G, B, A)
  ...
  Menu.Rectangle = nil
  Menu.Pagination = { Minimum = 1, Maximum = 13, Total = 13 }
  Menu.Safezone = true
...
function RageUI.CreateSubMenuStaff(
  ParentMenu,
  Title,
  Subtitle,
  X,
  ...
  if ParentMenu ~= nil then
    if ParentMenu() then
      local Menu = RageUI.CreateMenu(
        Title or ParentMenu.Title,
        Subtitle or string.upper(ParentMenu.Subtitle),
        1400,
        110
      )
      Menu.Parent = ParentMenu
...
function RageUI.Menus:EditSpriteColor(R, G, B, A)
  if self.Sprite.Dictionary == "utils" then
    self.Sprite.Color = {
      R = tonumber(R) or 255,
      G = tonumber(G) or 255,
      B = tonumber(B) or 255,
...
function RageUI.Menus:UpdateInstructionalButtons(Visible)
  ...
  buttons["↵"] = GetLabelText("HUD_INPUT2")

  if self.Closable then
    buttons["⬅"] = GetLabelText("HUD_INPUT3")
  end

  if self.InstructionalButtons ~= nil then
    for i = 1, #self.InstructionalButtons do
      if self.InstructionalButtons[i] and #self.InstructionalButtons[i] == 2 then
        local key = self.InstructionalButtons[i][1]:gsub("t_", "")
        buttons[key] = self.InstructionalButtons[i][2]
      end
    end
  end
end
