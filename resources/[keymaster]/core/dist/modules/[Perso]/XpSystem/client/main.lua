...
local disableNativeRankBar = SunnyConfigServ.XpSystems.HideNativeRankBar

if disableNativeRankBar then
...
function XNL_SetInitialXPLevels(CurrentXP, ShowRankBar, ShowRankBarAnimating)
    ...
    if ShowRankBar and not disableNativeRankBar then
...
local xpWeaponLockActive = false

local function setXpWeaponLock(state, message)
    if state and not xpWeaponLockActive then
        xpWeaponLockActive = true
        TriggerEvent('nv-weapons:toggleSafety', true, {
            enterMessage = message or "~r~Vous n'avez pas le niveau nécessaire.",
            blockedMessage = message or "~r~Vous n'avez pas le niveau nécessaire."
        })
        TriggerEvent('ox_inventory:disarm', true)
        SetCurrentPedWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), true)
    elseif not state and xpWeaponLockActive then
        xpWeaponLockActive = false
        TriggerEvent('nv-weapons:toggleSafety', false)
    end
end

Citizen.CreateThread(function()
    while true do
        if XNL_CurrentPlayerXP <= 600 and (not _CHASSE or not _CHASSE.inChasse) then
            setXpWeaponLock(true, "~r~Vous n'avez pas le niveau nécessaire pour dégainer.")
