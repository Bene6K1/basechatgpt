    ...
    while IsRagdollPut do
        ...
        if entityAlpha < 100 then
            ...
            if IsRagdollPut then
                ...
                ResetPedRagdollTimer(Player.playerPed)
                ragodoll = true
                ESX.ShowHelpNotification("appuyer sur [E] ou ~INPUT_JUMP~ pour ~p~vous relever~w~.")
                end
            end
        end    
    end
...
local CAM_HEIGHT = 0.4
local noirActive = false
local noirMoving = false
local noirHeight = 0.0
...
local function handleCinematicAnim()
    ...
    DisplayRadar(not noirActive)
    setupHealthArmour(noirActive and 3 or 0)
...
RegisterCommand('noir', function()
    ...
    while noirHeight > 0.0 or noirActive do
        for i = 0, 1.0, 1.0 do
            DrawRect(0.0, 0.0, 2.0, noirHeight, 0, 0, 0, 255)
            DrawRect(0.0, i, 2.0, noirHeight, 0, 0, 0, 255)
        end
...
function setWeaponTint(weaponTint)
    SetResourceKvp(("%s:weaponTint"):format(SunnyConfigServ.ServerName), tostring(weaponTint or 0))
    SetPedWeaponTintIndex(Player.playerPed, Player.weapon, tonumber(weaponTint) or 0)
    return value
end
...
function setWeaponKeybind(name, weaponHash, weaponLabel)
    if not name or not weaponHash or not weaponLabel then return end
    SetResourceKvp(('%s:weaponKeybinds%s'):format(kvpData, name), tostring(weaponHash))
    SetResourceKvp(('%s:weaponKeybinds%s:label'):format(kvpData, name), tostring(weaponLabel))
