while GetResourceState('inv') ~= 'started' do
    Wait(250)
end
...
Inventory.GetItemCount = function(src, item)
    return ox_inventory:GetItemCount(src, item, false, false)
end
...
Inventory.HasItem = function(src, item, metadata)
    if type(metadata) == 'table' then
        local items = ox_inventory:Search(src, 'count', item, metadata)
        ...
        local count = ox_inventory:GetItem(src, item, nil, true)
