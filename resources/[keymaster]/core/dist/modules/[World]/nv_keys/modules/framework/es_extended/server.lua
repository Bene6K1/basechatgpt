...
local function isCashAccount(accountType)
    if not accountType then return true end
    local lowered = string.lower(accountType)
    return lowered == 'cash' or lowered == 'money'
end
...
-- Framework.GetPlayerUniqueID(src)
-- Returns the unique ID (numeric) of the player for owned_vehicles.owner
---@param src number
---@return number | nil
Framework.GetPlayerUniqueID = function(src)
    local xPlayer = Framework.GetPlayer(src)
    if not xPlayer then return end
    return xPlayer.getUniqueID and xPlayer.getUniqueID() or xPlayer.UniqueID
end
...
Framework.GetAccountBalance = function(src, _type)
    ...
    local account = _type and xPlayer.getAccount(_type) or nil
    if not account then return 0 end
    return account.money or account.balance or 0
end
...
RegisterNetEvent("esx:playerLoaded", function(src)
    src = src or source
    TriggerEvent(Scriptname .. ":server:OnPlayerLoaded", src)
end)

RegisterNetEvent("esx:playerLogout", function(src)
    src = src or source
    TriggerEvent(Scriptname .. ":server:OnPlayerUnload", src)
end)
