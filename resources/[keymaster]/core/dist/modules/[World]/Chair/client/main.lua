...
local function IsSeatAvailable(coords, action)
	...
	for index, ped in pairs(GetNearbyPeds()) do
		if ped ~= playerPed and not IsPedAPlayer(ped) then
			...
			if dist < 0.8 then
				if action == 'sit' then
					if IsEntityPlayingAnyLayAnim(ped) or dist < 0.35 then
						return false
					end
...
local function StopSitting()
	...
	-- Afficher le /me RP
	TriggerEvent("nv_me:displayEntity", GetPlayerPed(-1), "se lève")
...
local function SitOnSeat(data)
	...
	if metadata.isSitting then
		...
		-- Afficher le /me RP
		TriggerEvent("nv_me:displayEntity", playerPed, "s'assoit sur une chaise")
...
local function LayDownOnBed(data)
	...
	-- Afficher le /me RP
	TriggerEvent("nv_me:displayEntity", playerPed, "s'allonge")
end
...
-- Fonction pour trouver la chaise la plus proche (Optimisée avec GetGamePool)
local function GetNearestChair()
	local coords = GetEntityCoords(GetPlayerPed(-1))
	local nearestEntity = nil
	local nearestDist = Config_Chair.Chair.MaxDetectionDist

    local pool = GetGamePool('CObject')
    
    for _, entity in ipairs(pool) do
        local model = GetEntityModel(entity)
        if Models[model] and Models[model].sit then
...
-- Thread pour détecter les chaises proches
Citizen.CreateThread(function()
	while Models == nil do
		Citizen.Wait(100)
	end
	...
	while true do
		...
		if not metadata.isSitting and not metadata.isLaying then
            -- Optimisation: Si on a déjà une chaise proche, on vérifie juste la distance
            if nearestChairEntity and DoesEntityExist(nearestChairEntity) then
                local dist = #(GetEntityCoords(GetPlayerPed(-1)) - GetEntityCoords(nearestChairEntity))
                if dist < 2.0 then
                    waitTime = 0
                    isNearChair = true
                else
                    -- Si on s'éloigne, on reset et on laisse le scan reprendre au prochain tour
                    nearestChairEntity = nil
                    isNearChair = false
                end
            else
                local chairEntity, chairDist = GetNearestChair()

                if chairEntity and chairDist < 2.0 then
                    waitTime = 0
                    nearestChairEntity = chairEntity
                    isNearChair = true
                else
                    nearestChairEntity = nil
                    isNearChair = false
...
-- Keybind E pour s'asseoir (seulement quand proche d'une chaise)
RegisterKeyMapping('sit', "S'asseoir sur une chaise", 'keyboard', 'E')
RegisterCommand('sit', function()
	if metadata.isSitting or metadata.isLaying then
		StopCurrentAction()
	elseif isNearChair and nearestChairEntity and DoesEntityExist(nearestChairEntity) then
		local model = GetEntityModel(nearestChairEntity)
		local skipSeeCheck = Models[model].sit.skipSeeCheck
		if skipSeeCheck or CanPlayerReachSeat(GetEntityCoords(nearestChairEntity), nearestChairEntity) then
			SitOnSeat({entity = nearestChairEntity, poly = false, sit = Models[model].sit, raycast = false})
		end
	end
end, false)
