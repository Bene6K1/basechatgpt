...
RegisterNetEvent('sunny:garage:loadPlayerVehicles', function(UniqueID)
    Garages:loadWithPlayerUniqueID(UniqueID)
end)
...
RegisterNetEvent('sunny:garage:edit', function(id, Table)
    ...
    MySQL.Async.execute('UPDATE garages SET name = @name, label = @label, type = @type, defaultpos = @defaultpos, deletepos = @deletepos, spawnpos = @spawnpos, headingspawnpos = @headingspawnpos, activeblip = @activeblip WHERE id = @id', {['@id'] = id, ['@name'] = Table.name, ['@label'] = Table.label, ['@type'] = Table.type, ['@defaultpos'] = json.encode(Table.defaultpos), ['@deletepos'] = json.encode(Table.deletepos), ['@spawnpos'] = json.encode(Table.spawnpos), ['@headingspawnpos'] = Table.headingspawnpos, ['@activeblip'] = Table.activeblip}, function()
...
ESX.RegisterServerCallback("sunny:garage:getType", function(source, cb, plate, properties)
    ...
    MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE plate = @plate', { ['@plate'] = plate }, function(result)
        ...
        local ownerMatch = (
            tonumber(vehicle.owner) == tonumber(xPlayer.UniqueID) or
            vehicle.owner == xPlayer.getJob().name or
            vehicle.owner == xPlayer.getJob2().name or
            tostring(vehicle.owner) == tostring(xPlayer.UniqueID)
        )
        ...
        elseif vehicle.type == 'boat' then
            cb('boat')
        elseif vehicle.type == 'aircraft' then
            cb('aircraft')
        else
            cb(false)
        end
...
RegisterNetEvent('sunny:garage:state', function(plate, value, type, id, properties)
    ...
    local stateValue = value and 1 or 0
                ...
                for k,v in pairs(Garages.vehicles) do
                    if tostring(v.plate) == tostring(plate) then
                        if Garages.vehicles[k] then
                            Garages.vehicles[k].state = stateValue
