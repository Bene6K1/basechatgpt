...
AddEventHandler("Sunny:vehicle:spawnVehicle", function(vehicleName, rentTime)
    ...
    rentEndTime = rentStartTime + (rentTime * 1000)
    rentTotalTime = rentTime * 1000
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if rentEndTime and rentTotalTime then
            local remainingTime = rentEndTime - GetGameTimer()
            if remainingTime > 0 then
                DrawProgressBarRent(remainingTime, rentTotalTime)
            else
...
function DrawProgressBarRent(remainingTime, totalTime)
    local percent = remainingTime / totalTime
    
    local barWidth = 0.145
    local barHeight = 0.015
    local barX = 0.085
    local barY = 0.785 
    
    DrawRect(barX, barY, barWidth, barHeight, 0, 0, 0, 180)
    
    local currentWidth = barWidth * percent
    local currentX = barX - (barWidth - currentWidth) / 2
    DrawRect(currentX, barY, currentWidth, barHeight, 46, 204, 113, 200)
    ...
    SetTextEntry("STRING")
    AddTextComponentString("Location: " .. timeText)
    DrawText(barX - 0.04, barY - 0.012)
end
