    ...
    while true do
        ...
        for k,v in pairs(Garages.garage_data.garages) do
            ...
            if distance < 1.5 then
                if not v.interactionPoint then
                    v.interactionPoint = CreateInteractionPoint(
                        vector3(v.defaultpos.x, v.defaultpos.y, v.defaultpos.z),
                        "GARAGE",
                        function() GaragesNUI:OpenGarageMenu(v.type, k, false, v.name) end,
                        { icon = 'fa-car', dist = 1.5 }
                    )
                end
            else
                if v.interactionPoint then
                    RemoveInteractionPoint(v.interactionPoint)
                    v.interactionPoint = nil
                end
            end

            if distance2 < 1.5 then
                if not v.deleteInteractionPoint then
                    v.deleteInteractionPoint = CreateInteractionPoint(
                        vector3(v.deletepos.x, v.deletepos.y, v.deletepos.z + 0.25),
                        "RANGER VÉHICULE",
                        function() Garages:rentVehicle(k, v.type) end,
                        { icon = 'fa-parking', dist = 1.5 }
                    )
                end
            else
                if v.deleteInteractionPoint then
                    RemoveInteractionPoint(v.deleteInteractionPoint)
                    v.deleteInteractionPoint = nil
                end
            end
        ...
        for k,v in pairs(Garages.garage_data.data.Pounds) do
            for i,p in pairs(Garages.garage_data.data.Pounds[k]) do
                ...
                if distance < 1.5 then
                    if not p.interactionPoint then
                        p.interactionPoint = CreateInteractionPoint(
                            vector3(p.position.x, p.position.y, p.position.z),
                            "FOURRIÈRE",
                            function() GaragesNUI:OpenGarageMenu(k, i, true, p.name or k) end,
                            { icon = 'fa-truck', dist = 1.5 }
                        )
                    end
                else
                    if p.interactionPoint then
                        RemoveInteractionPoint(p.interactionPoint)
                        p.interactionPoint = nil
                    end
                end
    ...
    while main do Wait(1)
            ...
            RageUI.Line(UTILS.ServerColor.r, UTILS.ServerColor.g, UTILS.ServerColor.b, UTILS.ServerColor.o)
              --  RageUI.Button('Gérer les véhicules d\'un joueur', nil, {}, true, {
               ...
               --              RageUI.Visible(playerInformations, true)
                ...
                -- end
           --  })
            RageUI.Button('Gérer les véhicules (joueur/group)', nil, {}, true, {
                onSelected = function()
                    KeyboardUtils.use('Entrée le (ID/Group)', function(data)
        ...
        RageUI.IsVisible(edit, function()
            RageUI.Button('Rechercher', nil, {}, true, {
                onSelected = function()
                    KeyboardUtils.use('Entrez le nom du garage', function(data)
                        if data == nil or data == '' then return end
                        local searchTerm = string.lower(data)
                        local foundGarage = nil
                        for k,v in pairs(Garages.garage_data.garages) do
                            local garageName = string.lower(v.name or '')
                            local garageLabel = string.lower(v.label or '')
                            if string.find(garageName, searchTerm, 1, true) or string.find(garageLabel, searchTerm, 1, true) then
                                foundGarage = v
                                break
                            end
                        end
                        if foundGarage then
                            Garages.selectedId = foundGarage.id
                            if Garages.garage_data.garages[foundGarage.id].activeblip == 1 or Garages.garage_data.garages[foundGarage.id].activeblip == true then 
                                Garages.garage_data.garages[foundGarage.id].activeblip = true 
                            else 
                                Garages.garage_data.garages[foundGarage.id].activeblip = false 
                            end
                            RageUI.Visible(edit_select, true)
                            ESX.ShowNotification(('~g~Garage trouvé: %s'):format(foundGarage.label or foundGarage.name))
                        else
                            ESX.ShowNotification('~r~Aucun garage trouvé avec ce nom')
                        end
                    end)
                end
            })
        ...
        RageUI.IsVisible(edit_select, function()
            for k,v in pairs(Garages.garage_data.garages) do
                ...
                --     end
                -- })
                RageUI.Button('TP GARAGES', nil, {}, true, {
                    onSelected = function()
                        local garage = Garages.garage_data.garages[Garages.selectedId]
                        if garage and garage.defaultpos then
                            local playerPed = PlayerPedId()
                            SetEntityCoords(playerPed, garage.defaultpos.x, garage.defaultpos.y, garage.defaultpos.z)
                            ESX.ShowNotification('~g~Téléportation au point d\'accès du garage effectuée')
                        else
                            ESX.ShowNotification('~r~Erreur: Position du garage introuvable')
                        end
                    end
                })
                RageUI.Button('TP RANGER', nil, {}, true, {
                    onSelected = function()
                        local garage = Garages.garage_data.garages[Garages.selectedId]
                        if garage and garage.deletepos then
                            local playerPed = PlayerPedId()
                            SetEntityCoords(playerPed, garage.deletepos.x, garage.deletepos.y, garage.deletepos.z)
                            ESX.ShowNotification('~g~Téléportation au point de rangement effectuée')
                        else
                            ESX.ShowNotification('~r~Erreur: Position du point de rangement introuvable')
                        end
                    end
                })
                for k,v in pairs(Garages.garage_data.garages) do
            ...
            --     end
            -- })
            RageUI.Button('Supprimer le garage', nil, {}, Garages.selectedId ~= nil and (Garages.selectedId ~= 1 and Garages.selectedId ~= 2 and Garages.selectedId ~= 3), {
                onSelected = function()
                    local garageId = Garages.selectedId
                    local garage = Garages.garage_data.garages[garageId]
                    
                    if not garageId or not garage then
                        ESX.ShowNotification("~r~Erreur: Garage introuvable")
                        return
                    end

                    exports['prompt']:createPrompt(
                        SunnyConfigServ.ServerName,
                        'Confirmer la suppression du garage',
                        ('Voulez-vous confirmer la suppression du garage "%s" ?\n~r~Cette action est irréversible~s~'):format(garage.label or garage.name or 'Inconnu'),
                        function(response)
                            if response then
                                if Garages.garage_data.blips[garageId] and Garages.garage_data.blips[garageId].id then
                                    RemoveBlip(Garages.garage_data.blips[garageId].id)
                                    Garages.garage_data.blips[garageId] = nil
                                end
                                ...
                                TriggerServerEvent('sunny:garage:delete', garageId)
                                Garages.garage_data.garages[garageId] = nil
                                ESX.ShowNotification("~g~Garage supprimé avec succès")
                                RageUI.GoBack()
                            else -- Non
                                ESX.ShowNotification("Vous avez annulé la suppression du garage")
                            end
                        end
                    )
                end
            ...
            for k,v in pairs(Garages.garage_data.garages[Garages.selectedId].spawnpos) do
                ...
                RageUI.Button(('%s %s %s (%s)'):format(v.x, v.y, v.z, v.h or 90.0), '\n[ENTER] pour changer la position\n[E] pour supprimer la position\n', {}, true, {
                    ...
                    onSelected = function()
                        ...
                        Garages.garage_data.garages[Garages.selectedId].spawnpos[k].z = Garages.garage_data.garages[Garages.selectedId].spawnpos[k].z - 1
       ...
       RageUI.IsVisible(entrepriseInformations, function()
        ...
        if not RageUI.Visible(main) and not RageUI.Visible(create) and not RageUI.Visible(edit) and not RageUI.Visible(edit_select) and not RageUI.Visible(playerInformations) and not RageUI.Visible(editSpawnPos) and not RageUI.Visible(main) then main = RMenu:DeleteType('main') end
    end
end
...
function Garages:openMenu(id, type)
    ...
    while main do Wait(1)
        ...
        if not RageUI.Visible(main) and not RageUI.Visible(my_vehicle_group) and not RageUI.Visible(my_vehicle_group_select) and
...
function Garages:openPoundMenu(type, spawnPoint, pound, id)
    ...
    while main do Wait(1)
        ...
        if not RageUI.Visible(main) and not RageUI.Visible(my_vehicle_group) and not RageUI.Visible(vehiclePoundListPosition) and
            not RageUI.Visible(my_vehicle) and
            not RageUI.Visible(main) then
            main = RMenu:DeleteType('main')
            FreezeEntityPosition(PlayerPedId(), false)
...
function Garages:sortVehicle(vehicles, veh, defaultpos, defautheading, plate, garageid, type, k, p)
    local coords = vector3(defaultpos.x, defaultpos.y, defaultpos.z)
    local head = defaultpos.h or defaultpos.heading
    if not head then
    ...
    if type == 'pound' then
        ...
        local function r()
            ...
            if not ESX.Game.IsSpawnPointClear(coords, 2) then 
                local newPos = Garages.garage_data.data.Pounds['car'][p].pos[math.random(1, #Garages.garage_data.data.Pounds['car'][p].pos)]
                coords = newPos.spawnPoint
                head = newPos.heading
                if not head then
                    head = GetEntityHeading(PlayerPedId())
                end
    ...
    if type ~= 'pound' then
        ...
        local function recurse()
            ...
            if not ESX.Game.IsSpawnPointClear(coords, 2) then 
                local newSpawnPos = Garages.garage_data.garages[garageid].spawnpos[math.random(1, #Garages.garage_data.garages[garageid].spawnpos)]
                coords = vector3(newSpawnPos.x, newSpawnPos.y, newSpawnPos.z)
                head = newSpawnPos.h or newSpawnPos.heading
                if not head then
                    head = GetEntityHeading(PlayerPedId())
                end
            end
            ...
            return ESX.Game.IsSpawnPointClear(coords, 2) == true or recurse();
        end
...
function Garages:rentVehicle(id, type)
    ...
    ESX.TriggerServerCallback('sunny:garage:getType', function(cb)
        ...
        if not Garages.garage_data.garages[id] then
            ESX.ShowNotification('~r~Erreur: garage introuvable')
            return
        end
        
        if Garages.garage_data.garages[id].type ~= cb then 
            ESX.ShowNotification('~r~Ce type de véhicule n\'est pas accepté dans ce type de garage !') 
            return 
        end
