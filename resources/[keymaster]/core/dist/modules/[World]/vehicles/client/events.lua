...
AddEventHandler("player:enteredVehicle", function (vehicle)
    if GetPedInVehicleSeat(vehicle, -1) ~= Player.playerPed then return end
    if IsVehicleDriveable(vehicle, false) then return end

    local engineHealth = GetVehicleEngineHealth(vehicle)
    local safeguard = (VEHICLE_DAMAGE and VEHICLE_DAMAGE.engineSafeGuard) or 350
    if engineHealth <= safeguard + 1 then return end

    local plate = trimPlate(GetVehicleNumberPlateText(vehicle))
    if plate == '' then return end

    ESX.TriggerServerCallback('sunny:garage:requestPlayerCars', function(isOwner)
        ...
        if type(v) == 'table' then
            for k2,v2 in pairs(v) do
                AddZones(("extras_menu-%s-%s"):format(k, k2), {
                    ...
                    ActionText = 'Configurer les extras',
                    Action = function()
