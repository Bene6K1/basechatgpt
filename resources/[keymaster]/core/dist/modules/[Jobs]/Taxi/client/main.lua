...
function taxi:openMenu()
    ...
    while main do Wait(1)
        RageUI.IsVisible(main, function()
            RageUI.Checkbox('Statut de l\'entreprise', 'Quand cette case est cochée votre ENTREPRISE est notée comme ouverte', Society and Society.List and ESX.PlayerData and ESX.PlayerData.job and ESX.PlayerData.job.name and Society.List[ESX.PlayerData.job.name] and Society.List[ESX.PlayerData.job.name].state or false, {}, {
                onChecked = function()
                    ...
                    onSelected = function()
                        ...
                        KeyboardUtils.use('Montant', function(data)
                            ...
                            if playerData and playerData.job and playerData.job.name then
                                TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), ('society_%s'):format(playerData.job.name), playerData.job.label, data)
                            end
                        end)
                    end
                })
        ...
        if not ActiveCooldOwnTaxi then
            ...
            Citizen.CreateThread(function()
                ...
                RageUI.Button('Faire une annonce personnaliser', nil, {}, true, {
                    onSelected = function()
                        local jobName = ESX.PlayerData and ESX.PlayerData.job and ESX.PlayerData.job.name or 'taxi'
...
RegisterCommand('taxi_menu', function()
    if not ESX.PlayerData or not ESX.PlayerData.job or ESX.PlayerData.job.name ~= 'taxi' then return end
	...
	if inMission then
		...
		Citizen.CreateThread(
			function()
				...
				local ped = CreatePed(1, model, pos, 50.0, true, true)
				...
				local playerVehicle = GetVehiclePedIsIn(pPed, false)
				if playerVehicle ~= 0 then
					TaskTurnPedToFaceEntity(ped, playerVehicle, 2000)
					Wait(2000)
					local availableSeat = -1
					for i = 0, GetVehicleMaxNumberOfPassengers(playerVehicle) - 1 do
					...
					if availableSeat ~= -1 then
						TaskEnterVehicle(ped, playerVehicle, -1, availableSeat, 1.0, 1, 0)
					else
						TaskEnterVehicle(ped, playerVehicle, -1, 1, 1.0, 1, 0)
					end
					
					local timeout = 0
					while not IsPedInAnyVehicle(ped, false) and inMission and timeout < 100 do
						timeout = timeout + 1
						Visual.Subtitle("Attendre le client", 500)
						Wait(500)
					end
					
					if timeout >= 100 and not IsPedInAnyVehicle(ped, false) then
						SetPedIntoVehicle(ped, playerVehicle, availableSeat ~= -1 and availableSeat or 1)
					end
				else
					TriggerEvent("esx:showNotification", "Vous devez être dans un véhicule pour cette mission", "error")
					FinishMission()
					return
				end
				...
				while dst > 10.0 and inMission do
					...
					if not IsPedInAnyVehicle(ped, false) then
						Visual.Subtitle("Le client a quitté le véhicule", 500)
						break
					end
