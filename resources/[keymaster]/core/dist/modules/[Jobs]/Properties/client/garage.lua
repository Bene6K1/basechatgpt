...
AddEventHandler('sunny:properties:loadVehicle', function(propertiesData, vehicleData)
    ...
    while Properties.garage.isIn do
        ...
        if garageExit <= 1 then
            DrawInstructionBarNotification(Properties.garage.interior[propertiesData.garageType].interior.x, Properties.garage.interior[propertiesData.garageType].interior.y, Properties.garage.interior[propertiesData.garageType].interior.z, "Appuyez sur [E] pour intéragir (garage exit)")
            if IsControlJustPressed(0, 54) then
