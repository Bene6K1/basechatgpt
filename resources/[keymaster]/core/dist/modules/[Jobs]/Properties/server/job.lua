RegisterNetEvent('sunny:properties:job:addPropertiesForPlayer', function(target, price, propertiesID, paidType, time)
    ...
    if paidType == 'cash' then
        local cashCount = exports.ox_inventory:GetItemCount(target, 'cash')
        if cashCount < price then 
            TriggerClientEvent('esx:showNotification', source, '💲 La personne ne possède pas les fonds nécessaires !') 
            TriggerClientEvent('esx:showNotification', target, '💲 Vous ne possedez pas les fonds nécessaires !') 
            return 
        end
        exports.ox_inventory:RemoveItem(target, 'cash', price)
    else
        if targetPlayer.getAccount(paidType).money < price then 
            TriggerClientEvent('esx:showNotification', source, '💲 La personne ne possède pas les fonds nécessaires !') 
            TriggerClientEvent('esx:showNotification', target, '💲 Vous ne possedez pas les fonds nécessaires !') 
            return 
        end
...
RegisterNetEvent('sunny:properties:delete', function(propertiesID)
    ...
    local propertyToDelete = Properties.PropertiesList[propertiesID]
    
    if propertyToDelete.playersIG then
        for playerSrc, pData in pairs(propertyToDelete.playersIG) do
            local targetPlayer = ESX.GetPlayerFromId(playerSrc)
            if targetPlayer then
                Properties:addPlayer(playerSrc, 0)
                TriggerClientEvent('sunny:properties:teleport', playerSrc, propertyToDelete.enter)
                TriggerClientEvent('sunny:properties:changePlayerSate', playerSrc, propertiesID, false)
                TriggerClientEvent('esx:showNotification', playerSrc, '~y~La propriété a été supprimée, vous avez été téléporté à l\'extérieur.')
            end
        end
    end
    ...
    MySQL.Async.execute('DELETE FROM properties WHERE propertiesID = @propertiesID', {
        ...
        TriggerClientEvent('esx:showNotification', source, ('~g~Propriété #%s supprimée avec succès'):format(propertiesID))
    end)
end)
