    ...
    __call = function(_, name, data)
        ...
        function society:havePermission(permissionName, playerGrade)
            ...
            if not society.permissions[permissionName] then
                society.permissions[permissionName] = {}
                return
            end
            if not society.permissions[permissionName][playerGrade] then return end
    ...
    MySQL.Async.fetchAll('SELECT * FROM society_data', {}, function(result)
        ...
        for k,v in pairs(Society.List) do
            MySQL.Async.fetchAll('SELECT * FROM job_grades WHERE job_name = @a', {['@a'] = v.name}, function(rs)
                ...
                if GetResourceState('ox_inventory') == 'started' and v.posCoffre then
                    local stashName = 'coffre_' .. v.name
                    local stashLabel = 'Coffre ' .. (v.label or v.name)
                    local coords = {x = v.posCoffre.x, y = v.posCoffre.y, z = v.posCoffre.z}
                    
                    exports.ox_inventory:RegisterStash(stashName, stashLabel, 50, 200000, nil, nil, coords)
                end
...
RegisterNetEvent('sunny:society:create', function(data)
    ...
    MySQL.Async.execute('INSERT INTO society_data (name, label, posCoffre, posBoss, permissions, blips, cloakroom, clothes, cloakpos) VALUES (@name, @label, @posCoffre, @posBoss, @permissions, @blips, @cloakroom, @clothes, @cloakpos)', {
        ...
        if GetResourceState('ox_inventory') == 'started' and data.posCoffre then
            local stashName = 'coffre_' .. data.name
            local stashLabel = 'Coffre ' .. (data.label or data.name)
            local coords = {x = data.posCoffre.x, y = data.posCoffre.y, z = data.posCoffre.z}
...
RegisterNetEvent('sunny:society:withDrawMoney', function(societyName, amount, type)
    ...
    if not xPlayer then return end
    if xPlayer.job.name ~= societyName then return end
...
ESX.RegisterServerCallback('sunny:society:getPlayerItems', function(source, cb)
    local xPlayer = ESX.GetPlayerFromId(source)
    if not xPlayer then 
        cb({})
        return 
        ...
        if inventory and inventory.items then
            for slot, item in pairs(inventory.items) do
                if item and item.name and item.count and item.count > 0 then
                    local itemLabel = item.metadata and item.metadata.label or item.label or item.name
                    table.insert(items, {
                        name = item.name,
                        label = itemLabel,
                        count = item.count,
...
RegisterNetEvent('sunny:society:addItem', function(societyName, itemData, count)
    ...
    local itemCount = 0
    if GetResourceState('ox_inventory') == 'started' then
        itemCount = exports.ox_inventory:Search(source, 'count', itemData.name) or 0
    else
        local inventoryItem = xPlayer.getInventoryItem(itemData.name)
        if not inventoryItem then return end
        itemCount = inventoryItem.count or 0
    end
    
    if itemCount < count then 
        return TriggerClientEvent('esx:showNotification', source, 'Vous n\'en possédez pas assez') 
    end
...
RegisterNetEvent('sunny:society:removeItem', function(societyName, itemData, count)
    ...
    local canCarry = true
    if GetResourceState('ox_inventory') == 'started' then
        canCarry = exports.ox_inventory:CanCarryItem(source, itemData.name, count)
    else
        canCarry = xPlayer.canCarryItem(itemData.name, count)
    end
    
    if not canCarry then 
        return TriggerClientEvent('esx:showNotification', source, "~r~Vous n'avez pas assez de place dans votre inventaire") 
    end
    
    local success = xPlayer.addInventoryItem(itemData.name, count)
    if not success then 
        return TriggerClientEvent('esx:showNotification', source, "~r~Impossible d'ajouter l'item à votre inventaire") 
    end
...
RegisterNetEvent('sunny:society:requestList')
AddEventHandler('sunny:society:requestList', function()
    local source = source
    TriggerClientEvent('sunny:society:sendList', source, Society.List)
end)