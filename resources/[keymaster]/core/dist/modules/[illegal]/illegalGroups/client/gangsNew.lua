                        ...
                        exports['prompt']:createPrompt(
                            ...
                            function(response)
                                if response then -- Oui
                                    ...
                                    ESX.ShowNotification('✅ Gang créé avec succès')
                                else -- Non
                                    ESX.ShowNotification('Création du gang annulée')
                                end
                            end
                        )
                    end
                    ...
                    exports['prompt']:createPrompt(
                        ...
                        function(response)
                            if response then -- Oui
                                ...
                                ESX.ShowNotification('✅ Gang supprimé avec succès')
                                RageUI.GoBack()
                            else -- Non 
                                ESX.ShowNotification('Suppression du gang annulée')
                            end
                        end
                    )
                end
...
function Gangs:openInteractMenu()
    ...
    local main_territories = RageUI.CreateSubMenu(main, '', 'Territoires Contrôlés')
    local main_orga_info = RageUI.CreateSubMenu(main, '', 'Informations Organisation')
    ...
    while main do Wait(1)
        ...
        RageUI.IsVisible(main, function()
            RageUI.Separator(('Organisation: ~g~%s~s~'):format(gangData and gangData.label or "N/A"))
            RageUI.Line()
            
            RageUI.Button('Informations Organisation', nil, {RightLabel = '→'}, true, {
                onSelected = function()
                    ESX.TriggerServerCallback("GetCrewExperience", function(experience)
                        CREW_XP_DATA = experience or {}
                    end)
                end
            }, main_orga_info)
            
            RageUI.Button('Territoires Contrôlés', nil, {RightLabel = '→'}, true, {
                onSelected = function()
                    ESX.TriggerServerCallback("GetCrewTerritories", function(territories)
                        CREW_TERRITORIES = territories or {}
                    end)
                end
            }, main_territories)
            
            RageUI.Button('Revendiquer le Territoire', nil, {RightLabel = '→'}, true, {
                onSelected = function()
                    print("[DEBUG] Clicked Revendiquer button")
                    local hasExport, result, territoryName = pcall(function() 
                        return exports['nv_drugs']:IsPlayerInTerritory() 
                    end)
                    
                    print("[DEBUG] Export call: Success=" .. tostring(hasExport) .. " InTerritory=" .. tostring(result) .. " Name=" .. tostring(territoryName))

                    if hasExport and result then
                        print("[DEBUG] Triggering server event for: " .. tostring(territoryName))
                        TriggerServerEvent("cDrugs:ClaimTerritory", territoryName)
                    else
                        -- Fallback or not in territory
                        if not result then
                             ESX.ShowNotification("~r~Vous n'êtes sur aucun territoire revendicable.")
                        else
                             print("Error calling nv_drugs export")
                        end
                    end
                end
            })

            RageUI.Button('Modifier la Couleur', "Format Hex (ex: #FF0000)", {RightLabel = '→'}, true, {
                onSelected = function()
                    KeyboardUtils.use('Code couleur Hex (ex: #FF0000)', function(data)
                        if data then
                            if string.match(data, "^#%x%x%x%x%x%x$") then
                                TriggerServerEvent("crew:updateColor", data)
                            else
                                ESX.ShowNotification("~r~Format invalide. Utilisez #RRGGBB")
                            end
                        end
                    end)
                end
        ...
        RageUI.IsVisible(main_orga_info, function()
            if CREW_XP_DATA and CREW_XP_DATA.name then
                RageUI.Separator(('Organisation: ~g~%s~s~'):format(CREW_XP_DATA.name))
                RageUI.Button('Niveau', nil, {RightLabel = '~b~'..tostring(CREW_XP_DATA.level or 1)..'~s~'}, false, {})
                RageUI.Button('Expérience', nil, {RightLabel = '~g~'..tostring(CREW_XP_DATA.xp or 0)..' XP~s~'}, false, {})
            else
                RageUI.Separator('~r~Aucune donnée disponible~s~')
            end
        end)

        RageUI.IsVisible(main_territories, function()
            if CREW_TERRITORIES and #CREW_TERRITORIES > 0 then
                for _, territory in pairs(CREW_TERRITORIES) do
                    RageUI.Button(territory.name, nil, {RightLabel = '~g~'..tostring(territory.points)..' pts~s~'}, false, {})
                end
            else
                RageUI.Separator('~r~Aucun territoire contrôlé~s~')
            end
        end)

        if not RageUI.Visible(main) and not RageUI.Visible(main_inventory) and not RageUI.Visible(main_territories) and not RageUI.Visible(main_recruitPlayer) and not RageUI.Visible(main_orga_info) then
            main = RMenu:DeleteType('main')
        end
    end
end
...
CreateThread(function()
    ...
    while true do
        ...
        if playerName then
            if not Player then Player = {} end -- Fix nil crash
            ...
            for k,v in pairs(Gangs.List) do
                if v.name == playerName then
                    ...
                    if distChest <= 15 or distManagement <= 15 then
                        ...
                        if distChest < 1 then
                            DrawInstructionBarNotification(v.positions.chest.x, v.positions.chest.y, v.positions.chest.z, "Appuyez sur [E] pour intéragir (Coffre)")
                        ...
                        if distManagement < 1 then
                            DrawInstructionBarNotification(v.positions.boss.x, v.positions.boss.y, v.positions.boss.z, "Appuyez sur [E] pour intéragir (Gestion)")
