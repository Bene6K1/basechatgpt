...
RegisterNetEvent('sunny:gangs:manage:demotePlayer', function(targetUniqueID)
    ...
    if targetPlayer then
        ...
        xTargetPlayer.setJob2('unemployed2', 0)
...
RegisterNetEvent('sunny:gangs:chest:items:add', function(gangName, v, number)
    ...
    local playerItemCount = exports.ox_inventory:Search(source, 'count', v.name) or 0
    if playerItemCount < number then 
    ...
    if not SunnyConfigServ.PermanentItem[v.name] then
        local removed = exports.ox_inventory:RemoveItem(source, v.name, number)
        if not removed or removed == 0 then
            return TriggerClientEvent('esx:showNotification', source, "Impossible de retirer l'objet de votre inventaire.")
        end
...
RegisterNetEvent('sunny:gangs:chest:items:remove', function(gangName, v, number)
    ...
    if not exports.ox_inventory:CanCarryItem(source, v.name, number) then
    ...
    if retval then
        exports.ox_inventory:AddItem(source, v.name, number)
...
RegisterNetEvent('sunny:gangs:chest:weapons:add', function(gangName, v, number)
    ...
    for slot, item in pairs(inventoryItems) do
        if item and item.name == string.lower(v.name) then
            weaponItem = item
            break
        end
    end
    ...
    if not weaponItem then
        TriggerClientEvent('esx:showNotification', source, "Vous n'avez pas cette arme dans votre inventaire.")
        return
    end

    local removed = exports.ox_inventory:RemoveItem(source, weaponItem.name, 1, weaponItem.metadata, weaponItem.slot)
    if not removed or removed == 0 then
        TriggerClientEvent('esx:showNotification', source, "Impossible de retirer l'arme de votre inventaire.")
        return
    end
...
RegisterNetEvent('sunny:gangs:chest:weapons:remove', function(gangName, v)
    ...
    if retval then
        if not exports.ox_inventory:CanCarryItem(source, string.lower(v.name), 1) then
            gang:addWeapon({
                name = v.name,
                label = v.label
            }, 1)
            TriggerClientEvent('esx:showNotification', source, "Inventaire plein.")
            return
        end

        exports.ox_inventory:AddItem(source, string.lower(v.name), 1, v.metadata or { ammo = v.ammo or 0 })
