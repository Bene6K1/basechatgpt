...
AddEventHandler('sunny:bank:braquage:rob', function(robb)
	...
	if Banks[robb] then
		...
		if (os.time() - bank.lastrobbed) < 600 and bank.lastrobbed ~= 0 then
			...
			TriggerClientEvent('esx:showNotification', source, _U('already_robbed') .. (600 - (os.time() - bank.lastrobbed)) .. _U('seconds'))
			return
		end
		...
		if rob == false then
			...
			if(cops >= 1)then
				...
				SetTimeout(300000, function()
					...
					if not robbers[savedSource] then
						return
					end

					local xPlayer = ESX.GetPlayerFromId(savedSource)
					if not xPlayer then
					...
					if robbers[savedSource] ~= robb then
						return
					end

					rob = false
					robbers[savedSource] = nil
					TriggerClientEvent('sunny:bank:braquage:robberycomplete', savedSource, robb)
					...
					sendLog(('Le Joueur (%s - %s) viens de récupérer (%s$)'):format(xPlayer.name, xPlayer.UniqueID, bank.reward), {
						...
						fields = {
							...
							{title = "Discord", subtitle = ESX.GetIdentifierFromId(savedSource, "discord:")},
						},
						channel = 'end_banque'
					})
				end)
