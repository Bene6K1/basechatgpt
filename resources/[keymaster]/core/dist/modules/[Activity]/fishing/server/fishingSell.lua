RegisterNetEvent('SunnyServ:fishing:sell', function(name, label, price, c)
    ...
    local inventoryCount = exports.ox_inventory:Search(source, 'count', name) or 0

    if inventoryCount < c then
        return(
            TriggerClientEvent('esx:showNotification', source, ('Vous ne possédez pas asser de ~r~%s~s~'):format(label))
        )
    end

    local removed = exports.ox_inventory:RemoveItem(source, name, c)
    if not removed or removed == 0 then
        TriggerClientEvent('esx:showNotification', source, "Erreur lors de la vente du poisson, réessayez.")
        return
    end
