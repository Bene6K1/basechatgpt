...
CreateThread(function()
    ...
    if Config_Farm.Job[ESX.PlayerData.job.name] then
        for _,z in pairs(Config_Farm.Farm) do
            for k,v in pairs(Config_Farm.Farm[_]) do
                if not Config_Farm.Items[_][k].marker then
                    local zone = Config_Farm.RandomZone[k.._]
                    if zone then
                        ESX.Game.SpawnLocalObject(Config_Farm.Items[_][k].props, vector3(zone.x, zone.y, zone.z-0.98), function(obj)
                            FreezeEntityPosition(obj, true)
                            Config_Farm.objSpawn[obj] = obj
                        end)
                    end
    ...
    while true do 
        ...
        if Config_Farm.Job[ESX.PlayerData.job.name] then
            for _,z in pairs(Config_Farm.Farm) do
                for k,v in pairs(Config_Farm.Farm[_]) do
                    -- Vérifier si la zone existe avant de calculer la distance
                    if Config_Farm.RandomZone[k.._] then
                        ...
                        if Config_Farm.Items[_] and Config_Farm.Items[_][k] then
                            ...
                            if distance < 1.5 then
                                DrawInstructionBarNotification(Config_Farm.RandomZone[k.._].x, Config_Farm.RandomZone[k.._].y, Config_Farm.RandomZone[k.._].z, ("pour intéragir"):format(UTILS.COLORCODE))
                                if IsControlJustPressed(1,51) then
                                    ...
                                    if not Config_Farm.Items[_][k].marker then
                                        DeletePropsFarm()
                                        -- Respawn le prop à la nouvelle position
                                        ESX.Game.SpawnLocalObject(Config_Farm.Items[_][k].props, vector3(Config_Farm.RandomZone[k.._].x, Config_Farm.RandomZone[k.._].y, Config_Farm.RandomZone[k.._].z-0.98), function(obj)
                                            FreezeEntityPosition(obj, true)
                                            Config_Farm.objSpawn[obj] = obj
                                        end)
