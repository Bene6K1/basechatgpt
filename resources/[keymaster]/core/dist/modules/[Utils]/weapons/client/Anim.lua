...
local forcedWeaponState = {
	active = false,
	blockedMessage = "~r~Action impossible ici.",
	lastNotify = 0,
	disarmedWeapon = false
}

local function showWeaponNotification(text, notifType)
	if not text or text == '' then return end

	local ok = pcall(function()
		exports.notif:Send(text, 3, notifType or 'info')
	end)
...
local function getPlayerPed()
	return (Player and Player.playerPed) or PlayerPedId()
end

local function disarmOxWeapon()
...
local function applyWeaponSafety(state, opts)
	forcedWeaponState.active = state and true or false
	forcedWeaponState.blockedMessage = (opts and opts.blockedMessage) or forcedWeaponState.blockedMessage
	forcedWeaponState.lastNotify = 0

	local ped = getPlayerPed()
	local hadWeapon = GetSelectedPedWeapon(ped) ~= unarmedHash

	if forcedWeaponState.active then
		forcedWeaponState.disarmedWeapon = hadWeapon

		if hadWeapon and opts and opts.enterMessage then
			showWeaponNotification(opts.enterMessage, 'info')
		end

		SetCurrentPedWeapon(ped, unarmedHash, true)
		ClearPedTasks(ped)
		currWeapon = unarmedHash
		holstered = true
		disarmOxWeapon()
	else
		if forcedWeaponState.disarmedWeapon and opts and opts.leaveMessage then
			showWeaponNotification(opts.leaveMessage, 'success')
		end
		forcedWeaponState.disarmedWeapon = false
	end
end

RegisterNetEvent('nv-weapons:toggleSafety', function(state, opts)
	applyWeaponSafety(state, opts)
end)

exports('ToggleWeaponSafety', applyWeaponSafety)

AddEventHandler('ox_inventory:currentWeapon', function(weaponData)
	if forcedWeaponState.active and weaponData then
	...
	while true do
		...
		if not IsEntityDead(Player.playerPed) and IsPedOnFoot(Player.playerPed)  then
			...
			if newWeap ~= currWeapon then
				...
				if forcedWeaponState.active and newWeap ~= unarmedHash then
					active = true
					...
					if (GetGameTimer() - forcedWeaponState.lastNotify) > 1500 then
						showWeaponNotification(forcedWeaponState.blockedMessage, 'error')
						forcedWeaponState.lastNotify = GetGameTimer()
					end
				...
				if weapons[newWeap] ~= nil and weapons[newWeap].parameters.onlyBag then
					...
					if bagIndex == 0 then
						...
						if IsPedInAnyVehicle(Player.playerPed, false) then
							vehicle = GetVehiclePedIsIn(Player.playerPed, false)
						else
							vehicle = GetFrontVehicle(Player.playerPed)
							if vehicle == 0 then
								local pedCoords = GetEntityCoords(Player.playerPed)
								vehicle = GetClosestVehicle(pedCoords.x, pedCoords.y, pedCoords.z, 6.0, 0, 70)
							end
						end
