...
-- Normalize sex/gender to numeric 0 (male) or 1 (female)
local function normalizeSex(val)
    ...
    elseif t == 'string' then
        local s = val:lower()
        if s == '1' or s == 'f' or s == 'female' then return 1 end
        return 0
...
function ApplySkin(skin, clothes)
	...
	local third_weight = ((Character['face_g_weight'] or 0) / 100) + 0.0
	local gpId = Character['grandParentsId'] or Character['grandparents'] or 0
	SetPedHeadBlendData(playerPed, Character['dad'] or 0, Character['mom'] or 0, gpId, Character['dad'] or 0, Character['mom'] or 0, gpId, face_weight, skin_weight, third_weight, false)
...
RegisterNetEvent('skinchanger:change', function(key, val)
    if key == 'sex' then
        val = normalizeSex(val)
    end
    Character[key] = val
    if key == 'grandparents' then
        Character['grandParentsId'] = val
    end
    if key == 'sex' then
...
RegisterNetEvent('skinchanger:loadSkin', function(skin, cb)
    local sex = normalizeSex(skin['sex'])
    skin['sex'] = sex
...
RegisterNetEvent('skinchanger:loadClothes', function(playerSkin, clothesSkin)
    local sex = normalizeSex(playerSkin['sex'])
    playerSkin['sex'] = sex
