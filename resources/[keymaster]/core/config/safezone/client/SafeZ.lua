...
local safeWeaponMessages = {
    enterMessage = "~b~SafeZone~s~ • Vos armes ont été rangées automatiquement",
    blockedMessage = "~r~SafeZone~s~ • Impossible de dégainer ici",
    leaveMessage = "~b~SafeZone~s~ • Vous pouvez de nouveau dégainer"
}
...
--    })
 ...
 --   end
    ...
    local function CanBypassSafeZone()
        local job = ESX.PlayerData.job
        local group = ESX.PlayerData.group

        if job and SunnyConfigSafe.SafeZone.bypassJob[job.name] then
            return true
        end

        if group and SunnyConfigSafe.SafeZone.bypassGroup[group] then
            return true
        end
        ...
        for k,v in pairs(SunnyConfigSafe.SafeZone.list) do
            SunnyConfigSafe.SafeZone.list[k]:onPointInOut(PolyZone.getPlayerPosition, function(isPointInside, point)
                if isPointInside then
                    ...
                    if not CanBypassSafeZone() then
                        NetworkSetFriendlyFireOption(false)
                        TriggerEvent('nv-weapons:toggleSafety', true, safeWeaponMessages)
                    end
                    ...
                    if not CanBypassSafeZone() then
                        ...
                        TriggerEvent('nv-weapons:toggleSafety', false, safeWeaponMessages)
                    end
                end
            end)
        end
