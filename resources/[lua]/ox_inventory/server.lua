...
require 'modules.clothing.server'  -- Module validation slots vêtements
...
local function fetchESX()
	if ESX or shared.framework ~= 'esx' then
		return ESX
	end
...
local function resolvePlayerTarget(target)
	if target == nil then
		return
	end
	...
	if esx.GetExtendedPlayers then
		for _, xPlayer in pairs(esx.GetExtendedPlayers()) do
			if xPlayer and xPlayer.UniqueID and xPlayer.UniqueID == numeric then
...
local function hasStaffAccess(source, minRank)
	if source == 0 then
		return true
	end
	...
	local rankList = esx.GroupsRankRelative or {}
	local rank = rankList[xPlayer.getGroup()] or 0
	minRank = minRank or 7
	...
	local bypassGroups = esx.ByPASS_GROUP or {}
	if not bypassGroups[xPlayer.getGroup()] then
		local ok, inService = pcall(function()
			return exports['core']:getService(source)
		end)

		if not ok or not inService then
			if esx.Notifi then
				esx.Notifi(source, 'Vous n\'êtes pas en service staff')
			end
			return false
		end
	end
...
function server.setPlayerInventory(player, data)
	...
	-- IMPORTANT : Prioriser la BDD ox_inventory plutôt que ESX
	-- ESX peut passer un inventaire vide ou incorrect qui écraserait l'inventaire ox_inventory
	local dbInventory = db.loadPlayer(player.identifier)
	
	-- Utiliser l'inventaire de la BDD si disponible, sinon utiliser celui passé par ESX
	if dbInventory and type(dbInventory) == 'table' and #dbInventory > 0 then
		data = dbInventory
	end
	...
	if type(data) == 'table' then
		...
		-- IMPORTANT : Toujours passer par convertInventory pour ajouter les comptes ESX
		-- Même si le format est correct, on doit vérifier et ajouter les items d'argent
	...
	local existingInv = Inventory(player.source)
	if existingInv then
		print(('Inventaire déjà existant pour le joueur %s (source: %s), mise à jour au lieu de création'):format(player.name, player.source))
		existingInv.player = server.setPlayerData(player)
		existingInv.player.ped = GetPlayerPed(player.source)
		if server.syncInventory then server.syncInventory(existingInv) end
		return
	end
	...
	if inventory and inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			elseif not item.weapon and server.UseItem then
				...
				-- should resolve issues, i.e. github.com/esx-framework/esx-legacy/commit/9fc382bbe0f5b96ff102dace73c424a53458c96e
...
RegisterNetEvent('ox_inventory:throwItem', function(slot, coords)
	...
	if itemData then
		...
		if item and item.allowedToDrop == false then
			return TriggerClientEvent('ox_lib:notify', _source, {type = 'error', description = 'Vous ne pouvez pas jeter cet item'})
		end
...
lib.addCommand({'additem', 'giveitem'}, {
	help = 'Gives an item to a player with the given unique ID',
	params = {
		{ name = 'target', type = 'number', help = 'Unique ID of the player' },
	...
	if source ~= 0 and not hasStaffAccess(source) then
		return
	end
	...
	if item then
		local targetId = resolvePlayerTarget(args.target)

		if not targetId then
			return warn(('Failed to resolve player with unique ID "%s" for giveitem command'):format(args.target))
		end

		local inventory = Inventory(targetId) --[[@as OxInventory]]

		if not inventory then
			return warn(('Failed to find inventory for unique ID "%s"'):format(args.target))
		end
		...
		if not success then
			return Citizen.Trace(('Failed to give %sx %s to unique ID %s (%s)'):format(count, item.name, args.target, response))
		end

		source = Inventory(source) or { label = 'console', owner = 'console' }

		if server.loglevel > 0 then
			lib.logger(source.owner, 'admin', ('"%s" gave %sx %s to "%s" (UniqueID: %s)'):format(source.label, count, item.name, inventory.label, args.target))
		end
	end
end)
