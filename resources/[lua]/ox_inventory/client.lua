...
require 'modules.performance.client'
...
function client.openInventory(inv, data)
	...
	if inv == 'dumpster' and cache.vehicle then
		ESX.ShowNotification('~r~' .. locale('inventory_right_access'))
	end

	if not canOpenInventory() then
        ESX.ShowNotification('~r~' .. locale('inventory_player_access'))
    end
    ...
    if inv == 'shop' and invOpen == false then
        if cache.vehicle then
            ESX.ShowNotification('~r~' .. locale('cannot_perform'))
        end

        left, right, accessError = lib.callback.await('ox_inventory:openShop', 50, data)
    elseif inv == 'crafting' then
        if cache.vehicle then
            ESX.ShowNotification('~r~' .. locale('cannot_perform'))
        end

        left, right, accessError = lib.callback.await('ox_inventory:openCraftingBench', 50, data.id, data.index)
    ...
    elseif invOpen ~= nil then
        ...
        left, right, accessError = lib.callback.await('ox_inventory:openInventory', 50, inv, data)
    end

    if accessError then
        ESX.ShowNotification('~r~' .. locale(accessError))
    end
    ...
    SetInterval(client.interval, 50)
    ...
    SetTimeout(400, function()
        if invOpen and currentInventory then
            currentInventory.coords = GetEntityCoords(playerPed)
            currentInventory.ignoreSecurityChecks = nil
        end
    end)
	...
	if item and usingItem then
		...
		if success then
			if item.notification then
				ESX.ShowNotification(item.notification)
			end
...
local function useSlot(slot, noAnim)
	...
	if canUseItem(data.ammo and true) then
		if data.component and not currentWeapon then
			ESX.ShowNotification('~r~' .. locale('weapon_hand_required'))
		end
		...
		if durability and durability <= 100 and consume then
			if durability <= 0 then
				ESX.ShowNotification('~r~' .. locale('no_durability', label))
			elseif consume ~= 0 and consume < 1 and durability < consume * 100 then
				ESX.ShowNotification('~r~' .. locale('not_enough_durability', label))
			end
		end
		...
		elseif data.weapon then
            ...
            if data.hash ~= GetSelectedPedWeapon(playerPed) then
                ...
                ESX.ShowNotification('~r~' .. locale('cannot_use', data.label))
            end
			...
			elseif data.component then
				...
				for componentIndex = 1, #weaponComponents do
					if componentType == Items[weaponComponents[componentIndex]].type then
						ESX.ShowNotification('~r~' .. locale('component_slot_occupied', componentType))
					end
				end
				...
				for i = 1, #components do
					...
					if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
						if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
							ESX.ShowNotification('~r~' .. locale('component_has', label))
						else
				...
				ESX.ShowNotification('~r~' .. locale('component_invalid', label))
...
local function registerCommands()
	...
	local primary = { getCurrentKey = function() return nil end }

	local function togglePrimaryInventory()
		if invOpen then
			return client.closeInventory()
		end
	...
	RegisterCommand('oxinv_toggle', function()
		togglePrimaryInventory()
	end, false)
	
	RegisterKeyMapping('oxinv_toggle', locale('open_player_inventory'), 'keyboard', client.keys[1])
	
	primary = { getCurrentKey = function() return client.keys[1] end }
		...
		onPressed = function(self)
			...
			if currentWeapon.ammo then
				if currentWeapon.metadata.durability > 0 then
					...
					ESX.ShowNotification('~r~' .. locale('no_durability', currentWeapon.label))
				end
			end
		end
	})

	if client.keys[3] and client.keys[3] ~= client.keys[1] then
		lib.addKeybind({
			name = 'hotbar',
		...
		warn(('hotbar keybind disabled (hotbar key "%s" matches inventory key)'):format(client.keys[3] or 'nil'))
	end
...
function client.closeInventory(server)
	...
	if invOpen then
		...
		if GetResourceState('neva_clothes') == 'started' then
			TriggerEvent('neva_clothes:clearPreview')
		end
        ...
        if item then
            ...
            if count < 0 then
                if shared.framework == 'esx' then
                    TriggerEvent('esx:removeInventoryItem', item)
                end
            ...
            elseif count > 0 then
                if shared.framework == 'esx' then
                    TriggerEvent('esx:addInventoryItem', item)
                end
...
RegisterNetEvent('ox_inventory:inventoryReturned', function(data)
	...
	ESX.ShowNotification(locale('items_returned'))
	client.closeInventory()
...
RegisterNetEvent('ox_inventory:inventoryConfiscated', function(message)
	if source == '' then return end
	if message then ESX.ShowNotification(locale('items_confiscated')) end
	if currentWeapon then currentWeapon = Weapon.Disarm(currentWeapon) end
		...
		if point.isClosest and point.currentDistance < 1.2 then
			...
			if IsControlJustReleased(0, 38) then
				lib.callback('ox_inventory:buyLicense', 1000, function(success, message)
					if success ~= nil then
						local color = success == false and '~r~' or '~g~'
						ESX.ShowNotification(color .. locale(message, locale('license', point.type:gsub("^%l", string.upper))))
					end
				end, point.invId)
			end
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					...
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > maxDistance or (not client.hasGroup(shared.police) and not Player(currentInventory.id).state.canSteal) then
							client.closeInventory()
							ESX.ShowNotification('~r~' .. locale('inventory_lost_access'))
						else
...
RegisterNUICallback('previewClothingItem', function(data, cb)
	cb(true)

	if not data or not data.item then return end
	if GetResourceState('neva_clothes') ~= 'started' then return end

	TriggerEvent('neva_clothes:previewItem', data.item)
end)

RegisterNUICallback('clearClothingPreview', function(_, cb)
	cb(true)
...
RegisterNUICallback('removeComponent', function(data, cb)
	...
	if data.slot ~= currentWeapon.slot then
		ESX.ShowNotification('~r~' .. locale('weapon_hand_wrong'))
	end
...
local function renameSlot(slot)
	...
	if newName:len() < 3 or newName:len() > 50 then
		ESX.ShowNotification('~r~' .. locale('invalid_item_name'))
	end
...
local function giveItemToTarget(serverId, slotId, count)
    ...
    if notification then
        ESX.ShowNotification('~r~' .. locale(table.unpack(notification)))
    end
end
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	elseif response then
		if type(response) == 'table' then
			...
			ESX.ShowNotification('~r~' .. locale(response))
		end
	end
end)
...
RegisterNUICallback('buyItem', function(data, cb)
	...
	if message then
		if message.description then
			local color = message.type == 'error' and '~r~' or (message.type == 'success' and '~g~' or '')
			ESX.ShowNotification(color .. message.description)
		end
	end
...
RegisterNUICallback('craftItem', function(data, cb)
	...
	for i = 1, data.count do
		...
		if not success then
			if response then ESX.ShowNotification('~r~' .. locale(response or 'cannot_perform')) end
			break
		end
	end
