...
local function getFrontVehicle(ped)
...
local function requiresBag(groupHash)
	return groupHash == `GROUP_SMG`
		or groupHash == `GROUP_RIFLE`
		or groupHash == `GROUP_MG`
		or groupHash == `GROUP_SHOTGUN`
		or groupHash == `GROUP_SNIPER`
		or groupHash == `GROUP_HEAVY`
end

local function showServerNotif(msg)
...
local function canEquipConsideringBag(hash)
	local group = GetWeapontypeGroup(hash)
	if not requiresBag(group) then return true end
	if GetPedDrawableVariation(cache.ped, 5) ~= 0 then return true end
	if cache.vehicle or DoesEntityExist(getFrontVehicle(cache.ped)) then return true end
	showServerNotif('~r~Action Impossible~s~ : Vous devez disposer d\'un sac ou être proche d\'une voiture.')
	return false
end
...
function Weapon.Equip(item, data, noWeaponAnim)
	...
	if not canEquipConsideringBag(data.hash) then
