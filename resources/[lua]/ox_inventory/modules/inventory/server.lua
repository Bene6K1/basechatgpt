...
function Inventory.Save(inv)
    ...
    if inv.player then
        -- IMPORTANT : Toujours sauvegarder pour ESX aussi
        -- ESX utilise users.inventory pour stocker l'inventaire ox_inventory
...
local function dropItem(source, playerInventory, fromData, data)
	...
	if item and (item.allowedToDrop == false or item.allowedToTrade == false) then
		return false, 'item_cannot_drop'
	end
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromData then
			...
			if GetResourceState('neva_clothes') == 'started' then
				local valid, errorMsg = exports.ox_inventory:ValidateClothingSlot(fromData.name, data.toSlot)
				if not valid then
					TriggerClientEvent('ox_lib:notify', source, {
						type = 'error',
						description = errorMsg or 'Action non autorisée'
					})
					return false
				end
			end
...
local function prepareInventorySave(inv, buffer, time)
    ...
    if inv.player then
        -- IMPORTANT : Sauvegarder pour TOUS les frameworks, y compris ESX
        -- Cette ligne bloquait la sauvegarde pour ESX !
