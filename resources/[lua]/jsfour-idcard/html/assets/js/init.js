  ...
  window.addEventListener('message', function( event ) {
    if (event.data.action == 'open') {
      ...
      if ( type == 'driver' || type == null) {
        $('img').show();
        $('#name').css('color', '#e9edf1');

        var sexNorm = String(sex).toLowerCase().trim();
        var isMale = (sexNorm === 'm' || sexNorm === 'male' || sexNorm === 'homme' || sexNorm === '0' || sexNorm === 'h' || sexNorm === 'man');
        if ( isMale ) {
        ...
        if ( type == 'driver' ) {
          ...
          Object.keys(licenseData).forEach(function(key) {
            ...
            if ( type == 'bike' || type == 'truck' || type == 'car' ) {
              $('#licenses').append('<p class="badge-' + type + '">'+ type +'</p>');
            }
          });
        }
        ...
        if (type == 'fishing') {
          ...
          Object.keys(licenseData).forEach(function(key) {
            ...
            if ( type == 'fishing_mer') {
              type = 'fishing_mer';
            } else if ( type == 'fishing_douce' ) {
              type = 'fishing_douce';
            } else if ( type == 'fishing_ocean' ) {
              type = 'fishing_ocean';
            }

            if ( type == 'fishing_mer' || type == 'fishing_douce' || type == 'fishing_ocean' ) {
              $('#licenses').append('<p class="badge-' + type + '">'+ table[type] +'</p>');
            }
          });
          ...
          $('#height').text(userData.height);
          $('#signature').text(userData.firstname + ' ' + userData.lastname);
          $('#id-card').css('background', 'url(assets/images/fishing.png)');
        }
      }
